...
export default {
  ...
  //     return this.capHuyens.content.filter(
  //       (commune) => commune.parent === "60b87fb59adb921904a0213e"
  //     );
  //   },
  // },
  data() {
  ...
  methods: {
    ...
    //   this.filteredCommunes = this.capHuyens.content.filter(
    //     (commune) => commune.parent === "60b87fb59adb921904a0213e"
    //   );
    // },
        ...
        if (!this.isVPDK) {
          ...
          this.filteredCommunes.forEach((item) => {
            ...
            let shortName = item.name.replace(
              /(UBND|Ủy\s+ban\s+nhân\s+dân)\s+(huyện|thị\s+xã|thành\s+phố)\s+/i,
              ""
            );
            let tenTat = shortName.toLowerCase().replace(/\s+/g, "");

            item.tenTat = tenTat;
            let nameWithoutDienKhanh = this.name.replace("Diên Khánh ", "");
            let words = shortName.split(' ');
            let lastTwoWords = words.slice(-2).join(' ');
            item.tenQT = nameWithoutDienKhanh + " (" + lastTwoWords + ")";
            item.maQT = this.code + "-" + tenTat.replace("chinhánhvănphòngđăngkýđấtđai","");
            item.logoId = "";
          });
        } else {
          this.filteredCommunes = this.capHuyens.content.filter(
            (commune) => commune.parent === "6412df10ab3cb7219354aff2"
          );
          this.filteredCommunes.forEach((item) => {
            item.editing = false;

            let shortName = item.name.replace(
              /(UBND|Ủy\s+ban\s+nhân\s+dân|Chi\s+nhánh\s+Văn\s+phòng\s+Đăng\s+ký\s+đất\s+đai)\s+(huyện|thị\s+xã|thành\s+phố)\s+/i,
              ""
            );
            let tenTat = shortName.toLowerCase().replace(/\s+/g, "");
    ...
    async getProcessInfo2(processId) {
      try {
        // Gọi API clone quy trình
        // this.getHuyen();
        const apiUrl = `https://api.vpub.khanhhoa.gov.vn/bpm/process-definition/${processId}`;
    ...
    async cloneProcess2() {
      ...
      try {
        for (let process of this.selectedCommunes) {
          ...
          await this.getProcessInfo2(this.processCloneId);
        ...
        for (const task of this.definedTask) {
          ...
          if (taskDefinitionKey != "root") {
            ...
            try {
              ...
              //   };
              // }
              console.log("Vô đây", process);
              let childItem = this.capHuyens.content.find(
                (item) => item.parent === process.id
              );
              if (!childItem) {
                console.log("Tìm không ra");
                return 0;
              }
